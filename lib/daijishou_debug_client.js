...
DaijishouDebugClient.prototype.swapRunningOperation = function(operation) {
    const replacedOperation = this._runningOperation
    this._runningOperation = operation
    return replacedOperation
}
...
DaijishouDebugClient.prototype.writeFileToTcpSocketFromFile = function(basePath, relativeFilePath, callback) {
    ...
    const microOperation = {
        onReadable: () => {
            ...
            else if(statusCode==this.FILES_SUB_OPERATION_CHECKSUM_MISMATCHED_RESULT_STATUS_BYTE_CODE) {
...
DaijishouDebugClient.prototype.readFileFromTcpSocketToFile = function(basePath, callback) {
    const tcpClient = this._tcpClient;
    ...
    var fileSizeInBytes = null
    var readFileSizeInBytes = 0n

    var checksumBytes = null
    ...
    const resetAndRetry = () => {
        ...
        readFileSizeInBytes = 0n
    
        checksumBytes = null
    }

    const finalize = () => {
        checksumBytes = tcpClient.read(32)
        // console.log(checksumBytes);
        if(checksumBytes == null) return
        if(hash.digest().equals(checksumBytes)) {
            // console.log("Checksum passed.");
            tcpClient.write(Buffer.from([this.FILE_OPERATION_HEADER_BYTE, this.FILES_SUB_OPERATION_SUCCEED_RESULT_STATUS_BYTE_CODE]));
            this._runningOperation = replacedOperation;
            writableStream.close();
            callback(null)
        }
        else {
            log("Checksum mismatched.");
            resetAndRetry();
            tcpClient.write(Buffer.from([this.FILE_OPERATION_HEADER_BYTE, this.FILES_SUB_OPERATION_CHECKSUM_MISMATCHED_RESULT_STATUS_BYTE_CODE]));
        }
    }

    const microOperation = {
        onReadable: () => {
            if(relativeFilePath == null) relativeFilePath = this.readStringFromTcpSocket()
            if(relativeFilePath == null) return
            if(absoluteFilePath == null) absoluteFilePath = path.join(basePath, relativeFilePath)
            if(writableStream == null) writableStream = fs.createWriteStream(absoluteFilePath);
            if(fileSizeInBytes == null) fileSizeInBytes = this.readLongFromTcpSocket()
            if(fileSizeInBytes == null) return
            // console.log(relativeFilePath);
            // console.log(fileSizeInBytes);
            // console.log(readFileSizeInBytes);
            // console.log(pathName)
            // console.log(absoluteFilePath)
            if(readFileSizeInBytes >= fileSizeInBytes) {
                finalize()
                return
            }
            if(pathName==null&&!fs.existsSync(path.dirname(absoluteFilePath))) {
                pathName = path.dirname(absoluteFilePath)
                // console.log("pathName:"+ pathName)
                fs.mkdirSync(pathName, { recursive: true });
            }
            const chunk = tcpClient.read(fileSizeInBytes-readFileSizeInBytes);
            if(chunk == null) return
            writableStream.write(chunk);
            hash.update(chunk);
            // console.log(chunk.length);
            readFileSizeInBytes += BigInt(chunk.length)
            // console.log(readFileSizeInBytes);
            // console.log(readFileSizeInBytes >= fileSizeInBytes);
            if(readFileSizeInBytes >= fileSizeInBytes) {
                finalize()
                return
            }
...
DaijishouDebugClient.prototype.FILES_SUB_OPERATION_CHECKSUM_MISMATCHED_RESULT_STATUS_BYTE_CODE = 0x05;
...
DaijishouDebugClient.prototype.buildFilePushFileSubOperation = function(localPath, RemoteUriString) {
    ...
    const operation = {
        ...
        run: (callback)=> {
            cb = callback
            this._runningOperation = operation;
            // console.log("buildFilePushFileSubOperation run.");
            const next = () => {
                // console.log("buildFilePushFileSubOperation next.");
                ...
                // console.log("buildFilePushFileSubOperation end.");
            }
...
DaijishouDebugClient.prototype.buildFilePullFileSubOperation = function(RemoteUriString, localPath) {
    const headerByteCode = this.FILE_OPERATION_HEADER_BYTE;
    const tcpClient = this._tcpClient
    ...
    const startPulling = () => {
        ...
        const readNextFile = () => {
            ...
            this.readFileFromTcpSocketToFile(localPath, (err) => {
                if(err) {
                    cb(err);
                    this.disconnectTCP();
                    return
                }
                // console.log("readFileFromTcpSocketToFile");
                readNextFile();
            });
        }
        readNextFile();
    ...
    const operation = {
        ...
        run: (callback)=> {
            ...
            // console.log(12345);
            this._runningOperation = operation;
            tcpClient.write(Buffer.from([headerByteCode, this.PULL_FILE_SUB_OPERATION_BYTE_CODE]));
            // console.log(54321);
            this.writeStringToTcpSocket(RemoteUriString);
        },
        onReadable: firstOnReadable,
...
DaijishouDebugClient.prototype.console = function(command, callback) {
    this.ensureConnection((err) => {
        ...
        this.addTaskToExecutionQueue((err, finish) => {
            ...
            // console.log("before buildConsoleOperation")
...
DaijishouDebugClient.prototype.pullFile = function(RemoteUriString, localPath, callback) {
    this.ensureConnection((err) => {
        ...
        this.addTaskToExecutionQueue((err, finish) => {
            // console.log("addTaskToExecutionQueue");
            if(err) {
                callback(err, null);
                return
            }
            this.buildFilePullFileSubOperation(RemoteUriString, localPath).run((err, result) => {
                if(callback) callback(err, result);
                finish();
            });
        });
